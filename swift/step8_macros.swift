...
class MalList: MalType {
    ...
    func nth(n: MalInt) -> MalType {
        if n.value < items.count {
            return items[n.value]
        } else {
            return MalError(value: "IndexOutOfBoundsException")
        }
    }
}
...
func isMacroCall(ast: MalType, env: Env) -> Bool {
    if ast is MalList && (ast as MalList).first() is MalSymbol {
        let sym = ((ast as MalList).first() as MalSymbol).value
        let fun = env.get(sym)
        if fun is MalFun && (fun as MalFun).isMacro == true {
            return true
        } else {
            return false
        }
...
func macroExpand(ast: MalType, env: Env) -> MalType {
    var ast = ast
    while isMacroCall(ast, env) {
        let mac = env.get(((ast as MalList).first() as MalSymbol).value) as MalFun
        let result = mac.fn((ast as MalList).rest())
        ast = macroExpand(result, env)
    }
    return ast
}
              ...
              case is MalSymbol:
                ...
                case "def!":
                    return env.set((items[1] as MalSymbol).value,
                        v: eval(items[2], env))
                case "defmacro!":
                    var fun = eval(items[2], env) as MalFun
                    fun.isMacro = true
                    return env.set((items[1] as MalSymbol).value,
                        v: fun)
                case "let*":
                    var newEnv = Env().outer(env)
                    var bindings: MalList
                    if items[1] is MalVec {
                        let items = (items[1] as MalVec).items
                        bindings = MalList(items: items)
                    } else {
                        bindings = items[1] as MalList
                    }
                    newEnv = createLetEnv(bindings, newEnv)
                    env = newEnv
                    ast = items[2]
                case "macroexpand":
                    return macroExpand(items[1], env)
                case "do":
                    let c = items.count - 1
                    for var i = 1; i < c; i++ {
                        eval(items[i], env)
...
let ns = [
    ...
    "nth": MalFun({(x: MalList) -> MalType in
        if x.first() is MalList && x.second() is MalInt {
            return (x.first() as MalList).nth((x.second() as MalInt))
        } else if x.first() is MalVec && x.second() is MalInt {
            return (x.first() as MalVec).nth((x.second() as MalInt))
        } else {
            return MalError(value: "wrong types to nth")
        }
    }),
    "first": MalFun({(x: MalList) -> MalType in
        if x.first() is MalList {
            return (x.first() as MalList).first()
        } else if x.first() is MalVec {
            return (x.first() as MalVec).first()
        } else {
            return MalError(value: "first requires a list")
        }
    }),
    "rest": MalFun({(x: MalList) -> MalType in
        if x.first() is MalList {
            return (x.first() as MalList).rest()
        } else if x.first() is MalVec {
            return (x.first() as MalVec).rest()
        } else {
            return MalError(value: "rest requires a list")
        }
    }),
